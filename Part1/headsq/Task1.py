...
def main():
    # Create a volume reader for the head CT dataset
    ...
    # Set the input connection for the reader
    reader.Update()
    ...
    def keypress_callback(obj, event):
        ...
        if key == "Right":
            ...
            if current_mode[0] == 1:
                ...
                renderer.AddViewProp(skin)
                renderer.AddViewProp(bone)
                sliderWidget.SetEnabled(True)  # Enable slider in iso-surface extraction mode
                current_mode[0] = 0
    ...
    # Add keypress callback function to handle events
    interactor.AddObserver("KeyPressEvent", keypress_callback)

    # Callback function for adjusting the skin extractor value
    def keypress(obj, event, skin_extractor=skin_extractor):
        key = obj.GetKeySym()
        if key == "Up":
            current_value = skin_extractor.GetValue(0)
            skin_extractor.SetValue(0, current_value + 50)
        elif key == "Down":
            current_value = skin_extractor.GetValue(0)
            skin_extractor.SetValue(0, current_value - 50)
        obj.GetRenderWindow().Render()

    interactor.AddObserver("KeyPressEvent", lambda obj, event: keypress(obj, event, skin_extractor))
